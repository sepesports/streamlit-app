...
<head>
  ...
  <style>
    ...
    *{{
    ...
    .top-select-wrapper:hover, .top-btn:hover{{
      background:#f5f5f5;
    }}
    .top-select-wrapper.active, .top-btn.active{{
      background:#f5f5f5;
    }}
...
<script>
  (function () {{
    ...
    // ========== OBTENER DATOS REALES DESDE MALLAS (ALTAS) ==========
    async function fetchSocorristas() {{
      try {{
        ...
        data.rows.forEach(row => {{
          // Campo "Socorrista" según la hoja ALTAS
          const nombre = row["Socorrista"] || row["socorrista"] || "";
          if (nombre.trim()) socorristasSet.add(nombre.trim());
        }});
        // Devolvemos objetos con dni = nombre (pues no tenemos DNI en la hoja ALTAS)
        // Si en el futuro se agrega DNI, se puede modificar aquí.
    ...
    async function fetchInstalaciones() {{
      try {{
        ...
        data.rows.forEach(row => {{
          // Campo "Instalacion" según la hoja ALTAS
    ...
    // ========== FUNCIONES DE CHAT (sin cambios, ya funcionaban) ==========
