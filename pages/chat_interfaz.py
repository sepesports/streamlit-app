...
<script>
  (function () {{
    ...
    // ========== FUNCIONES REALES PARA OBTENER DATOS DE ALTAS ==========
    // REEMPLAZA LAS URLS CON TUS ENDPOINTS REALES.
    // DEBEN DEVOLVER UN ARRAY DE OBJETOS CON LA ESTRUCTURA:
    // Para socorristas: [{{ "dni": "12345678", "nombre": "Juan Pérez" }}, ...]
    // Para instalaciones: [{{ "id": "inst1", "nombre": "Piscina Municipal" }}, ...]
    async function fetchSocorristas() {{
      // TODO: Reemplazar con tu endpoint real
      const url = "https://tusitio.com/api/altas/socorristas";
      return await fetchJSON(url);
    }}

    async function fetchInstalaciones() {{
      // TODO: Reemplazar con tu endpoint real
      const url = "https://tusitio.com/api/altas/instalaciones";
      return await fetchJSON(url);
    }}
    // =================================================================
    ...
    async function loadDropdowns() {{
      try {{
        ...
        if (!Array.isArray(socorristas)) throw new Error("Respuesta inválida");
      ...
      try {{
        ...
        if (!Array.isArray(instalaciones)) throw new Error("Respuesta inválida");
