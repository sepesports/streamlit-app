...
# Versión final con depuración y escape seguro de variables Python -> JS
# ==============================================================================

# ------------------------------------------------------------------------------
# 1. OBTENER PARÁMETROS DE AUTENTICACIÓN (con compatibilidad)
# ------------------------------------------------------------------------------
query_params = st.query_params
AUTH_USER = query_params.get("usuario", [""])[0] if isinstance(query_params.get("usuario"), list) else query_params.get("usuario", "")
if not AUTH_USER:
    AUTH_USER = query_params.get("user", [""])[0] if isinstance(query_params.get("user"), list) else query_params.get("user", "")

AUTH_ROLE = query_params.get("rol", [""])[0] if isinstance(query_params.get("rol"), list) else query_params.get("rol", "")
if not AUTH_ROLE:
    AUTH_ROLE = query_params.get("role", [""])[0] if isinstance(query_params.get("role"), list) else query_params.get("role", "")

AUTH_DNI = query_params.get("dni", [""])[0] if isinstance(query_params.get("dni"), list) else query_params.get("dni", "")
...
# Si no hay autenticación, redirigir al login (admin)
...
# ------------------------------------------------------------------------------
# 2. CONFIGURACIÓN DE ESTILOS Y DISPOSICIÓN
# ------------------------------------------------------------------------------
PAD_X_PX = 10
PAD_TOP_PX = 10
BORDER_PX = 2
...
# ------------------------------------------------------------------------------
# 3. FUNCIÓN DE ESCAPADO SEGURO PARA JAVASCRIPT
# ------------------------------------------------------------------------------
def js_escape(s):
    """Escapa una cadena para ser usada como string literal en JavaScript (entre comillas dobles)."""
    if not isinstance(s, str):
        s = str(s)
    # Escapar barra invertida, comillas dobles, saltos de línea, backticks y ${}
    s = s.replace('\\', '\\\\')   # Barra invertida
    s = s.replace('"', '\\"')     # Comillas dobles
    s = s.replace('\n', '\\n')    # Nueva línea
    s = s.replace('\r', '\\r')    # Retorno de carro
    s = s.replace('\t', '\\t')    # Tabulador
    s = s.replace('`', '\\`')     # Backtick (para template literals)
    s = s.replace('${', '\\${')   # Interpolación de template literal
    return s

# ------------------------------------------------------------------------------
# 4. GENERAR EL HTML + JAVASCRIPT COMPLETO (CON LOGS DE DEPURACIÓN)
# ------------------------------------------------------------------------------
html_template = r"""
<!doctype html>
<html>
    ...
    <div id="wrap">
      ...
      <div id="agenda" class="panel agendaBlock">
        ...
        <div class="meta agendaMeta" id="agendaMeta">
          ...
          <div><b>Usuario:</b> <span id="userDisplay">--</span> (<span id="roleDisplay">--</span>)</div>
        </div>
...
<script>
// =============================================================================
// PARÁMETROS INYECTADOS DESDE PYTHON (ESCAPADOS SEGURAMENTE)
// =============================================================================
const AUTH_USER = "__AUTH_USER__";
const AUTH_ROLE = "__AUTH_ROLE__";
const AUTH_DNI  = "__AUTH_DNI__";
const IS_SOCORRISTA = (AUTH_ROLE.toLowerCase() === "socorrista");
const IS_ADMIN_OR_DIRECTIVO = (AUTH_ROLE.toLowerCase() === "administrador" || AUTH_ROLE.toLowerCase() === "directivo");

// Mostrar usuario y rol en la UI
document.getElementById('userDisplay').textContent = AUTH_USER;
document.getElementById('roleDisplay').textContent = AUTH_ROLE;

// =============================================================================
// LOGS DE DEPURACIÓN (CONSOLA)
// =============================================================================
console.log("=== INICIO CALENDARIO ===");
console.log("AUTH_USER:", AUTH_USER);
console.log("AUTH_ROLE:", AUTH_ROLE);
console.log("AUTH_DNI:", AUTH_DNI);
console.log("IS_SOCORRISTA:", IS_SOCORRISTA);
console.log("=========================");
...
function detectDNIColumn(row) {
  const possibleNames = ["DNI","dni","Cédula","cedula","Documento","documento","Cedula de ciudadania","Numero documento","Identificación","identificacion","ID","id"];
  ...
  console.warn("⚠️ No se encontró ninguna columna que parezca DNI.");
  return null;
}
...
function getDisplayStatus(row) {
  ...
  if (rawEstado.includes("disponible")) {
    ...
    if (socorrista.toLowerCase() === AUTH_USER.toLowerCase()) {
...
function renderCalendar(year, month) {
  ...
  if(monthDisplay) monthDisplay.textContent = new Date(year, month, 1).toLocaleDateString('es-ES', { month: 'long', year: 'numeric' }).toUpperCase();
}
...
function getFilteredRows(){
  ...
  return ALL_ROWS.filter(r => {
    ...
    if (IS_SOCORRISTA && AUTH_DNI) {
      ...
      if (rowDNI.toLowerCase() !== AUTH_DNI.toLowerCase()) return false;
    }
...
function rebuildAvailability(){
  ...
  ALL_ROWS.forEach(r => {
    ...
    if (IS_SOCORRISTA && AUTH_DNI) {
      ...
      if (rowDNI.toLowerCase() !== AUTH_DNI.toLowerCase()) return;
...
async function loadMallas(){
  ...
  console.log("Cargando datos desde", ENDPOINT_MALLAS);
  try{
    ...
    ALL_ROWS = data.rows;
    console.log(`✅ Se recibieron ${ALL_ROWS.length} filas.`);
    if (ALL_ROWS.length > 0) {
      console.log("Primera fila (muestra):", ALL_ROWS[0]);
      ...
      if (DNI_COLUMN_NAME) {
        ...
        console.warn("No se encontró columna DNI. El filtro por DNI no funcionará.");
      }
    } else {
      console.warn("⚠️ No hay filas en los datos.");
    }
    console.log("DNI del usuario autenticado:", AUTH_DNI);
    console.log("Rol:", AUTH_ROLE);
    // Construir lista de socorristas
    if (IS_SOCORRISTA) {
      const setS = new Set();
      ALL_ROWS.forEach(r => {
        let rowDNI = "";
        ...
        if (rowDNI.toLowerCase() === AUTH_DNI.toLowerCase()) {
    ...
    console.log("Socorristas únicos:", SOCORRISTAS);
...
// Modal (placeholder, sin backend por ahora)
...
if (optNovedad) optNovedad.addEventListener('change', function(e) { if(novedadInput) novedadInput.style.display = e.target.checked ? 'block' : 'none'; });
...
function init() {
  ...
  document.getElementById('btnModificar')?.addEventListener('click', function() { if (!this.disabled) showModal(); });
  const aplicarBtn = document.getElementById('btnAplicar');
  const modificarBtn = document.getElementById('btnModificar');
  if(aplicarBtn) aplicarBtn.disabled = true;
  if(modificarBtn) modificarBtn.disabled = true;
  renderCalendar(currentYear, currentMonth);
  updateAgenda();
  loadMallas();
}
...
# Reemplazar los placeholders de las variables inyectadas (escapadas)
html = html_template.replace("__AUTH_USER__", js_escape(AUTH_USER))
html = html.replace("__AUTH_ROLE__", js_escape(AUTH_ROLE))
html = html.replace("__AUTH_DNI__", js_escape(AUTH_DNI))

# Reemplazar placeholders de medidas (son números, no necesitan escape)
