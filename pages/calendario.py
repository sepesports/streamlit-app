          ...
          if (BTN_TEXTS[i] === "Horarios") {
            d.addEventListener("click", function(){
              try{
                ...
                params.set("auth", "ok");
                if (!params.get("usuario") && USER_NAME) {
                ...
                navigateTop("/calendario?" + params.toString());
              }catch(e){
                ...
                navigateTop("/calendario?" + fallback.toString());
              }
            });
          }
            ...
            if (CAN_REGISTER_USERS) {
              d.addEventListener("click", function(){
                try{
                  ...
                  navigateTop("/altas_registro?" + params.toString());
                }catch(e){
                  var fallback = new URLSearchParams();
                  fallback.set("auth", "ok");
                  if (USER_NAME) fallback.set("usuario", USER_NAME);
                  if (USER_ROLE) fallback.set("rol", USER_ROLE);
                  if (USER_DNI) fallback.set("dni", USER_DNI);
                  navigateTop("/altas_registro?" + fallback.toString());
                }
              });
            } else {
            ...
            if (CAN_MANAGE_SCHEDULES) {
              d.addEventListener("click", function(){
                try{
                  ...
                  navigateTop("/editar_horarios?" + params.toString());
                }catch(e){
                  var fallback = new URLSearchParams();
                  fallback.set("auth", "ok");
                  if (USER_NAME) fallback.set("usuario", USER_NAME);
                  if (USER_ROLE) fallback.set("rol", USER_ROLE);
                  if (USER_DNI) fallback.set("dni", USER_DNI);
                  navigateTop("/editar_horarios?" + fallback.toString());
                }
              });
            } else {
              d.classList.add("disabled");
